...
private fun DecorationBox(
    ...
    Column {
        ...
        Row(
            ...
            if(prefix != null) Box(propagateMinConstraints = true) {
                prefix()
            }

            Box(
                Modifier.widthIn(minWidth, maxWidth).weight(1f, false),
                propagateMinConstraints = true
            ) {
                if(placeholder != null && isEmpty()) placeholder()

                innerTextField()
            }

            if(suffix != null) Box(propagateMinConstraints = true) {
                suffix()
            }
        }
